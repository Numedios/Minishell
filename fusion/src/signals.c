...
/*   Updated: 2023/02/19 19:16:45 by zhamdouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//declarer g_exit_code[2], le deuxieme int servira a gerer les cas dans les enfants
void	handle_sig(int sig)//il faut free si exit
{
	if (sig == SIGINT) //ctrl+c
	{
		if (g_exit_code[1] == 9) //pour here doc 
		{
			...
			psk je me dis peut etre que je mettrait en place un g_exit_code special pour celui la, en mode si g_exit_code[1]=5, 
		...
		else
		{
			...
			rl_redisplay();
			g_exit_code[0] = 130;//verifier les codes erreurs
		}
	}
