...
{
	return (find_if_have_output(output, ">") != 1
		&& find_if_have_output(output, "<") != 1
		&& find_if_have_output(output, ">>") != 1
		&& find_if_have_output(output, "<<") != 1);
}
/*
int	pipex(t_maillons *maillons, char ***env, t_garbage *garbage)
{
	...
	if (len == 1)
	{
		if (check_no_sep(maillons->output) &&
		 str_cmp(maillons->command, "exit") && check_builtin(maillons->args) == 0)
...
int	pipex(t_maillons *maillons, char ***env, t_garbage *garbage)
{
	...
	if (len == 1)
	{
		...
		!= 1 && (check_echo(maillons->args, 0, 0, 1) == 0 ||
...
static int	check_if_exit_one(t_maillons *maillons)
{
	return (check_no_sep(maillons->output)
		&& (str_cmp(maillons->command, "exit"))
		&& check_builtin(maillons->args, 0) == 0);
}

static int	check_if_bultins_one(t_maillons *maillons)
{
	return (find_if_have_output(maillons-> output, ">") != 1
...
int	pipex(t_maillons *maillons, char ***env, t_garbage *garbage)
{
	...
	if (len == 1)
	{
		if (check_if_exit_one(maillons))
			do_exit(maillons->args[1], garbage, 0);
		else if (check_if_bultins_one(maillons))
		{
			check_if_builtin(garbage->new_env, \
			&(garbage->new_env), 0, garbage);
			check_echo(garbage->maillons->args, 0, 0, 0);
